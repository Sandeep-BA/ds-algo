...
object BS_FindPeakElement {
  ...
  def binarySearch(array: Array[Int]): Int = {
    ...
    while (start <= end) {
      ...
      if (mid > 0 && mid < size) if (array(mid) > array(mid - 1) &&
        array(mid) > array(mid + 1)) return mid
      else if (array(mid - 1) > array(mid)) end = mid - 1 else start = mid + 1
      else if (mid == 0) if (array(mid) > array(mid + 1)) return mid
      else return mid +1 else if (mid == size) if (array(mid) > array(mid -
        1)) return mid else return mid - 1
    }
    -1
  }
